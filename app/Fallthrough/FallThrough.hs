...
testActivityConcurrentPerActIMF f log 
  ...
  | otherwise = False
      where
        ...
        endAct = endActivities log
        atLeastTwoActivities = length allAct >= 2
        filteredLog = logFiltering f log
        allActFiltering = allActivities filteredLog
        allTransitionsFiltering = transitions filteredLog
        startActFiltering = startActivities filteredLog
        endActFiltering = endActivities filteredLog
